...
ROLE_ALIASES = {
    'Admin Level 1': 'Admin Ethical Level 1',
...
ROLE_FALLBACK_LOOKUP = {
    'Admin Ethical Level 1': ['Admin Level 1'],
...
def fetch_users_from_db():
    try:
        ...
        admin_roles = ('Admin', 'Admin Level 0', 'Admin Ethical Level 1', 'Admin Level 2', 'Ethical Staff Level 1', 'Finance Staff Level 2', 'Admin / Finance Staff', 'Finance Staff', 'SGF Manager')
...
def apply_so_history_notes_to_cart(customer_name: str) -> bool:
    """
    ...
    # New customer (no history) or no matches: lookup is empty - still fill with N/A
    updated = False
    for item in cart:
...
def get_orders_by_tag(rep_code, tag_type):
    ...
    tag_type: 'PMR_tag', 'TSR_tag', 'DSMBU7_tag', 'DSMPSI_tag', 'DSM' (both BU7 and SPI), or 'TSR_or_PMR' (TSR_tag OR PMR_tag).
    Common ID: user's rep_code must match the account tag (TSR_tag, PMR_tag, etc.) stored in the order when it was submitted."""
...
def format_order_status_display(order_row):
    """Format order status for display, showing Level 2 approval for TRADE accounts"""
    status = order_row.get('Status', 'N/A')
    approved_by_l1 = order_row.get('ApprovedByLevel1', '')
    
    # For TRADE accounts (SYSTEM approval), show status as "Pending (Level 2)"
...
Method 2: Set in code (Contact developer)
  Update GMAIL_ACCOUNT constant in Sales_Order_Inventory_App.py""")
        ...
        # Build CC list: from DB if not provided
        if isinstance(cc_emails, str):
            cc_list = [cc_emails]
        elif cc_emails:
            cc_list = list(cc_emails)
        else:
            cc_list = db.get_cc_emails()
        if cc_list:
    ...
    try:
        ...
        for rep_code in rep_codes_to_notify:
            ...
            if related_user_email:
                send_email_notification(related_user_email, subject, body)
...
def send_booking_request_notification_to_tsr(request_id, tsr_code, tsr_name, client_name, shipping_date, 
    ...
    try:
        ...
        if send_email_notification(tsr_email, subject, body):
            db.insert_notification_log(
                notification_type='booking_request_new',
                recipient_type='TSR',
                recipient_id=tsr_code,
...
def send_approval_notification_to_admin(order_id, order_data, admin_level):
    """
    Send notification to Admin Ethical Level 1 or Admin Level 2 when order needs their approval
    """
    try:
        # Get emails for the specific admin level (Level 1 = Admin Ethical Level 1)
        role = 'Admin Ethical Level 1' if admin_level == 1 else f'Admin Level {admin_level}'
        ...
        salutation = "Dear Admin Ethical Level 1," if admin_level == 1 else f"Dear Admin Level {admin_level},"
        body = build_notification_email("Order Approval Required", salutation, msg, "Review Order")
        
        # Send to all admins at this level
        for admin_email in admin_emails:
            if send_email_notification(admin_email, subject, body):
                db.insert_notification_log(
                    notification_type=f'approval_level{admin_level}',
                    recipient_type=role,
        ...
        try:
            _role = 'Admin Ethical Level 1' if admin_level == 1 else f'Admin Level {admin_level}'
            db.insert_notification_log(
                notification_type=f'approval_level{admin_level}',
...
def count_pending_approvals(admin_level):
    ...
    if admin_level == 1:
        ...
        # Exclude TRADE accounts (ApprovedByLevel1 = 'SYSTEM') - they skip Level 1
        pending_count = len(orders_df[
            (orders_df['Status'] == 'Pending') & 
            (orders_df['ApprovedByLevel1'] == '')
        ])
    elif admin_level == 2:
        # Count orders that are Pending, approved by Level 1 (or SYSTEM for TRADE), but not yet by Level 2
        pending_count = len(orders_df[
            (orders_df['Status'] == 'Pending') & 
            (orders_df['ApprovedByLevel1'] != '') &
            (orders_df['ApprovedByLevel2'] == '')
...
def send_sgf_notification():
    ...
    try:
        ...
        result = send_email_notification(sgf_email, subject, body, sender_email)
        if result:
...
def send_approval_notification(admin_level):
    ...
    try:
        ...
        result = send_email_notification(to_email, subject, body, sender_email)
        if result:
...
def resend_notification_for_order(order_id):
    ...
    if status == 'Pending for SGF' and not approved_by_sgf:
        ...
        log_email_notification("INFO", f"Sending Admin Ethical Level 1 notification for order {order_id}")
        result = send_approval_notification(admin_level=1)
        if result:
            return True, "Admin Ethical Level 1 notification sent successfully"
        else:
            return False, "Failed to send Admin Ethical Level 1 notification"
...
def registration_page():
    ...
    with st.form("registration_form"):
        ...
        email = st.text_input("Email Address *", key="reg_email",
                              help="Email address for order notifications (required)")
...
def submit_order_dialog():
    ...
    if not st.session_state.cart:
        ...
        st.info("✏️ Fill in **Notes/Remarks** for each SKU in the table below. Click **💾 Save Changes (Notes /Remarks)** after editing before submitting the order.")
        ...
        if update_cart and not edited_df.empty:
            # CRITICAL: Preserve account tags before rerun (needed for notifications even if not displayed)
            ...
            # Restore account tags and client selection after cart update (before rerun)
        ...
        st.markdown("---")
        # Request Booking checkbox - above Client Information, used to filter account list (Sales Rep only)
        request_booking = False
        if st.session_state.get('authenticated', False):
            current_role = st.session_state.get('user_role', '')
            is_sales_rep = current_role == 'Sales Rep'
            is_tsr = current_role == 'TSR'
            if is_tsr:
                request_booking = False
            else:
                if 'dialog_request_booking' not in st.session_state:
                    st.session_state.dialog_request_booking = is_sales_rep
        ...
        if not client_name:
            ...
            with st.form("booking_request_form_dialog"):
                ...
                if uploaded_files_br:
                    ...
                    for uploaded_file in uploaded_files_br:
                        ...
                        if file_size_mb > 5:
                            invalid_files_br.append(f"{uploaded_file.name} ({file_size_mb:.2f} MB - exceeds 5MB limit)")
                        else:
            ...
            with st.form("order_form_dialog"):
                ...
                col1, col2 = st.columns(2)
                with col1:
                    rep_code = st.text_input("Code *", value=st.session_state.rep_code, key="dialog_rep_code")
                    rep_name = st.text_input("Name *", value=st.session_state.rep_name, key="dialog_rep_name")
                    rep_company = st.text_input("Company *", value=st.session_state.rep_company, key="dialog_rep_company")
                with col2:
                    rep_dept = st.text_input("Dept/DSM District *", value=st.session_state.rep_dept, key="dialog_rep_dept")
                    rep_area = st.text_input("Area/PMR *", value=st.session_state.rep_area, key="dialog_rep_area")
                    ...
                    # Validation - ensure client_name is set (redundant when form is shown, but kept for safety)
                        ...
                        if missing_fields:
                            ...
                            with st.spinner("Submitting order... Saving data and sending notifications."):
                                ...
                                is_trade_account = account_type.upper() == 'TRADE'
                                
                                # For TRADE accounts: Skip Level 1, go directly to Level 2
                                # For Dispensing accounts: Normal flow (Level 1 then Level 2)
                                if needs_sgf:
                                    initial_status = 'Pending for SGF'
                                    approved_by_l1 = ''
                                    approved_date_l1 = ''
                                elif is_trade_account:
                                    # TRADE accounts skip Level 1 approval
                                    initial_status = 'Pending'
                                    approved_by_l1 = 'SYSTEM'  # Mark as auto-approved by system
                                    approved_date_l1 = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                else:
                                    # Dispensing accounts go through Level 1 first
                                    initial_status = 'Pending'
                                    approved_by_l1 = ''
                                    approved_date_l1 = ''
                            ...
                            if save_orders(orders_df):
                                ...
                                if needs_sgf:
                                    ...
                                    status_message = "Pending for Level 2 approval (TRADE account - skips Level 1)"
                                else:
...
def manage_users_dialog():
    ...
    with st.expander("➕ Add / Edit User", expanded=st.session_state.user_to_edit is not None):
        with st.form("user_form"):
            ...
            role_options = ["Sales Rep", "TSR", "Admin Level 0", "Admin Ethical Level 1", "Admin Level 2", "Ethical Staff Level 1", "Finance Staff Level 2", "SGF Manager"]
            ...
            role = st.selectbox("Role *", role_options, index=role_index,
                                help="Sales Rep = standard sales UI. TSR = different UI (Booking Request, TRADE flow, Related Orders by TSR tag). Admin Level 0 = Super Admin; Admin Ethical Level 1/Admin Level 2 = approvers; Ethical Staff Level 1/Finance Staff Level 2 = view-only.")
            ...
            col1, col2 = st.columns(2)
            with col1:
                rep_code = st.text_input("Code", value=edit_data.get('RepCode', ''))
                rep_name = st.text_input("Name", value=edit_data.get('RepName', ''))
                rep_company = st.text_input("Company", value=edit_data.get('RepCompany', ''))
            with col2:
                rep_dept = st.text_input("Dept", value=edit_data.get('RepDept', ''))
                rep_area = st.text_input("Area", value=edit_data.get('RepArea', ''))
...
def notification_management_dialog():
            ...
            if st.button("📧 Sample Sending Notification", key="sample_notification_btn", use_container_width=True,
                ...
                try:
                    ...
                    samples = [
                        ...
                        ("[Sample] Order ORD-SAMPLE - Pending Level 1 Approval", build_notification_email("Order Approval Required", "Dear Admin Ethical Level 1,", m3, "Review Order")),
                        ...
                        ("[Sample] Reminder: Order ORD-SAMPLE - Pending Level 1 Approval", build_notification_email("Approval Reminder", "Dear Admin Ethical Level 1,", m6, "Review Orders")),
    ...
    with tab_cc:
        ...
        if cc_df.empty:
            cc_df = pd.DataFrame(columns=['id', 'email', 'display_order'])
            cc_df = pd.concat([cc_df, pd.DataFrame([{'id': None, 'email': '', 'display_order': 0}])], ignore_index=True)
...
def accounts_dialog():
    ...
    with st.expander("📤 Bulk Upload Accounts", expanded=False):
        ...
        with col_up:
            uploaded_accounts_file = st.file_uploader("Upload CSV", type=['csv'], key="bulk_upload_accounts_csv",
                                                     help="Upload filled template to bulk add/update accounts")
            ...
            process_accounts_btn = st.button("Process Upload", type="primary", key="bulk_upload_accounts_process",
                                            use_container_width=True, help="Click to process the uploaded CSV file")
                            ...
                            for idx, row in upload_df.iterrows():
                                ...
                                for col in all_cols:
                                    ...
                                    if col == 'Active':
                                        ...
                                        v = str(val).strip() or 'Dispensing'
                                        new_row[col] = v if v in ('Dispensing', 'TRADE', 'Distribution', 'Contract') else 'Dispensing'
                                    else:
    ...
    if accounts_df.empty:
        ...
        if available_cols:
            ...
            if st.session_state.get('highlight_new_account_row', False):
                ...
                st.markdown("""
                    <style>
                    /* Highlight the new row in the accounts data editor (sorts to top when Customer name is empty) */
...
def sales_rep_interface():
            ...
            try:
                ...
                if not active_accounts.empty and client_name_br:
                    ...
                    if not client_row.empty:
                        ...
                        st.session_state.client_description = str(cd.get('Customer code', '')).strip()
                        st.session_state.client_mobile = str(cd.get('Contact number1', '')).strip()
                        ba = str(cd.get('Business address', '')).strip()
                        st.session_state.billing_address = ba
                        st.session_state.shipping_address = ba
                        st.session_state.contact_person_1 = str(cd.get('Contact person1', '')).strip()
                        st.session_state.payment_terms = str(cd.get('Credit term', '')).strip()
                ...
                # Special instructions map to both Notes and Delivery Instructions (TSR can edit if blank)
                special_instr = (br.get('special_instructions') or '').strip()
                st.session_state.notes = special_instr
                st.session_state.remarks = br.get('remarks', '')
                st.session_state.delivery_terms = special_instr  # Pre-fill so TSR can submit; fallback enables if still blank
    ...
    # Determine tag type for Related Submitted Orders (Med Reps, TSR, DSM)
    # Common ID: user's rep_code must match account tag (TSR_tag, PMR_tag, etc.) stored in order
    rep_code = st.session_state.get('rep_code', '')
    _tag_type = None
    if account_type == 'TRADE':
        _tag_type = 'TSR_or_PMR'  # TSR sees orders where rep_code matches TSR_tag OR PMR_tag
    elif account_type == 'Dispensing':
        _tag_type = 'PMR_tag'
    elif account_type == 'Distribution':
        _tag_type = 'DSM'
    show_related_orders = _tag_type is not None and rep_code
                    ...
                    with st.container(border=True):
                        ...
                        with col_btn:
                            ...
                            if add_to_cart_clicked:
                                ...
                                # Step 4: Single rerun - no success message, no extra operations
                                st.rerun()
            else:
    ...
    with tab2:
        ...
        if st.session_state.get('order_submission_success', False) and st.session_state.get('last_submitted_order_id'):
            st.success(f"✅ Order {st.session_state.last_submitted_order_id} has been successfully submitted with status 'Pending'! Your cart has been cleared. Check the Request/Order History tab to view your order.")
        ...
        if not st.session_state.cart:
            ...
            if is_completing_br:
                ...
                # Update cart items with edited notes_remarks only when Save Changes button is clicked
                if update_cart and not edited_df.empty:
                    # CRITICAL: Preserve form states before rerun
                    # Save current client selection, checkbox state, and account tags BEFORE any changes
                    ...
                    # CRITICAL: Restore form states IMMEDIATELY after cart update (before rerun)
                    # This ensures the states persist across the rerun, including account tags
                    if current_client and current_client != '':
                        st.session_state.client_name_select = current_client
                    if current_request_booking:
                        st.session_state.tab_request_booking = True
                    # Restore account tags (needed for notifications even if not displayed)
                    if current_TSR_tag:
                        st.session_state.account_TSR_tag = current_TSR_tag
                    if current_PMR_tag:
                        st.session_state.account_PMR_tag = current_PMR_tag
                ...
                if st.session_state.get('authenticated', False):
                    ...
                    if is_tsr:
                        ...
                        if 'tab_request_booking' not in st.session_state:
                            st.session_state.tab_request_booking = is_sales_rep
                        request_booking = st.checkbox(
                            "Request Booking (TRADE - Med Rep requests TSR to complete order)",
            ...
            if not client_name:
                ...
                with st.form("order_form"):
                    # Client fields - when TSR completing booking: all read-only (automatic complete view)
                    # Only Remarks is editable for TSR Complete Booking
                    ...
                    with col1:
                        ...
                        contact_person_1_mobile = st.text_input("Contact Person 1 Mobile", key="contact_person_1_mobile", disabled=_field_disabled("contact_person_1_mobile"))
                    with col2:
                        contact_person_2 = st.text_input("Contact Person 2", key="contact_person_2", disabled=_field_disabled("contact_person_2"))
                        contact_person_2_mobile = st.text_input("Contact Person 2 Mobile", key="contact_person_2_mobile", disabled=_field_disabled("contact_person_2_mobile"))
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        rep_code = st.text_input("Code *", value=st.session_state.rep_code, key="rep_code")
                        rep_name = st.text_input("Name *", value=st.session_state.rep_name, key="rep_name")
                        rep_company = st.text_input("Company *", value=st.session_state.rep_company, key="rep_company")
                    with col2:
                        rep_dept = st.text_input("Dept/DSM District *", value=st.session_state.rep_dept, key="rep_dept")
                        rep_area = st.text_input("Area/PMR *", value=st.session_state.rep_area, key="rep_area")
                        ...
                        if not client_name or client_name.strip() == '':
                            ...
                            if missing_fields:
                                ...
                                with st.spinner("Submitting order... Saving data and sending notifications."):
                                    ...
                                    account_type = get_account_type_by_client_name(client_name)
                                    is_trade_account = account_type.upper() == 'TRADE'
                                    # is_trade_booking already set above before validation
                                    
                                    # Determine approval workflow based on account type
                                    if needs_sgf:
                                        initial_status = 'Pending for SGF'
                                        approved_by_l1 = ''
                                        approved_date_l1 = ''
                                    elif is_trade_account or is_trade_booking:
                                        # TRADE accounts: Skip Level 1, go directly to Level 2
                                        initial_status = 'Pending'
                                        approved_by_l1 = 'SYSTEM'  # Mark as auto-approved by system
                                        approved_date_l1 = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                    else:
                                        # Dispensing accounts: Normal flow (Level 1 then Level 2)
                                        initial_status = 'Pending'
                                        approved_by_l1 = ''
                                        approved_date_l1 = ''
    ...
    with tab3:
        ...
        # Show My Booking Requests (for Sales Reps who submitted booking requests) - exclude Auto-Cancel
        br_created_df = db.get_booking_requests_by_created_by(st.session_state.get('username', ''))
        br_created_df = br_created_df[br_created_df['status'] != 'Auto-Cancel'] if not br_created_df.empty else br_created_df
        ...
        if not br_auto_cancel_df.empty:
            ...
            st.caption("Booking requests cancelled after 24 hours without completion. View details below.")
                        ...
                        with col1:
                            ...
                            if approved_by_l1:
                                if approved_by_l1 == 'SYSTEM':
                                    st.info(f"✅ TRADE Account - Auto-approved by System (skips Level 1)")
                                else:
        ...
        _can_view_so_history = normalize_role(st.session_state.get('user_role', '')) in ('Admin Level 0', 'Admin Ethical Level 1', 'Admin Level 2') or st.session_state.get('username') == 'administrator'
    ...
    # Related Submitted Orders tab (Med Reps, TSR, DSM - filtered by tag matching rep_code)
    if tab4 is not None:
        with tab4:
            st.header("Related Submitted Orders")
            st.caption("Orders submitted for accounts tagged with your rep code (TSR_tag, PMR_tag, or DSM tag). Ensure accounts in List of Accounts have your rep code in the matching tag column.")
            related_orders = get_orders_by_tag(rep_code, _tag_type)
            if related_orders.empty:
                st.info("No related orders found. Orders appear here when accounts you are tagged to (PMR/TSR/DSM) have submitted orders.")
            else:
...
def finance_staff_interface():
    ...
    with st.sidebar:
        ...
        # Show "Manage Users" and "Manage Products" for Admin Ethical Level 1 or Admin / Finance Staff (excluding view-only)
        ...
        is_finance_staff = user_role in ('Admin / Finance Staff', 'Finance Staff', 'Admin Level 0', 'Admin Ethical Level 1', 'Admin Level 2', 'Ethical Staff Level 1', 'Finance Staff Level 2')
    ...
    # Main Content - Tabs
    tab1, tab2, tab3, tab4_so = st.tabs(["📋 All Pending Orders Review", "📊 All Orders", "📜 Approval History", "📊 SO History"])
    ...
    with tab1:
        ...
        if orders_df.empty:
            ...
            if admin_level == 1:
                # Admin Ethical Level 1 sees orders that are Pending (not yet reviewed by Level 1)
                # Exclude TRADE accounts (ApprovedByLevel1 = 'SYSTEM') - they skip Level 1
                    ...
                    with st.container():
                        ...
                        with col3:
                            ...
                            if view_only:
                                ...
                                # Show "Unlock for Edit" for Admin Ethical Level 1 or Super Admin (Level 0)
                    ...
                    with col3:
                        ...
                        with col_approve:
                            if st.button("✅ Approve", key=f"sgf_approve_{row['OrderID']}", type="primary"):
                                ...
                                order_idx = orders_df[orders_df['OrderID'] == row['OrderID']].index
                                if len(order_idx) > 0:
                                    # Mark as approved by SGF, change status to Pending for Admin Ethical Level 1
                                    ...
                                    if save_orders(orders_df):
                                        # Send email notification to Admin Ethical Level 1
                                        try:
                                            send_approval_notification(admin_level=1)
                                        except Exception as e:
                                            print(f"Error sending notification email: {e}")
                                        
                                        st.success(f"Order {row['OrderID']} approved by SGF Manager! Proceeding to Admin Ethical Level 1.")
                                        st.rerun()
...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        if user_role in ('Sales Rep', 'TSR'):
            sales_rep_interface()
        elif user_role in ('Admin Level 0', 'Admin Ethical Level 1', 'Admin Level 2', 'Ethical Staff Level 1', 'Finance Staff Level 2', 'Admin / Finance Staff', 'Finance Staff'):
